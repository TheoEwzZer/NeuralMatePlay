...
def test_pin_handling(network: DualHeadNetwork, results: TestResults) -> float:
    ...
    for test in TEST_POSITIONS:
        ...
        if test["pin_type"] == "evaluation":
            ...
            for i, idx in enumerate(top_5):
                ...
                if move:
                    uci: str = move.uci()
                    is_pin: bool = uci in expected_set
                    if is_pin and found_at is None:
                        found_at = i + 1
                    pin_str: str = "PIN!" if is_pin else ""
                    color: str = Colors.GREEN if is_pin else ""
